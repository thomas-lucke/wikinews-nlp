...
from src.data_inspector import load_raw_records  # noqa: F401  (re-exported for monkeypatching in tests)
...
def _select_from_categories(
    categories: list[str], selected_labels: list[str]
) -> Optional[str]:
    ...
    normalised_categories = {
        _normalise_topic_string(c) for c in categories if isinstance(c, str)
    }
...
def print_normalisation_summary(
    valid: list[dict], dropped: list[DroppedRecord]
) -> None:
    ...
    for idx, raw in enumerate(raw_records):
        ...
        if not isinstance(text, str):
            ...
            if inferred_key is not None:
                ...
                working["text"] = text
                logger.info(
                    "Record %d: inferred text from field %r", idx, inferred_key
                )
