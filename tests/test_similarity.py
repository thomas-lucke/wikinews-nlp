...
def test_score_all_articles_per_article_error_does_not_crash(
    ...
    try:
        ...
        pytest.fail(f"score_all_articles must catch per-article errors, got: {exc!r}")
    assert "similarity_score" not in out[0]
