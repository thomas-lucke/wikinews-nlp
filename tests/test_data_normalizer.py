...
def test_field_mappings_precedence_text_wins_over_body(tmp_path):
    ...
    articles, _ = data_normalizer.normalise_articles(
        raw_path=str(p), detected_format="jsonl",
        languages=["en"], topics=["Sports"], countries=["Germany"],
        max_per_topic=10, min_article_length=100, random_seed=42,
    )
...
def test_wikinews_categories_resolve_to_configured_topic(tmp_path):
    ...
    articles, _ = data_normalizer.normalise_articles(
        raw_path=str(p), detected_format="jsonl",
        languages=["en"], topics=["Politics and conflicts"], countries=["Germany"],
        max_per_topic=10, min_article_length=100, random_seed=42,
    )
    assert len(articles) == 1
...
def test_dropped_record_language_not_in_config(tmp_path):
    ...
    _articles, dropped = data_normalizer.normalise_articles(
        raw_path=str(p), detected_format="jsonl",
        languages=["en", "de"], topics=["Sports"], countries=["Germany"],
        max_per_topic=10, min_article_length=100, random_seed=42,
    )
...
def test_dropped_record_country_not_in_config(tmp_path):
    ...
    _articles, dropped = data_normalizer.normalise_articles(
        raw_path=str(p), detected_format="jsonl",
        languages=["en"], topics=["Sports"], countries=["Germany", "United States"],
        max_per_topic=10, min_article_length=100, random_seed=42,
    )
...
def test_dropped_record_topic_not_in_config(tmp_path):
    ...
    _articles, dropped = data_normalizer.normalise_articles(
        raw_path=str(p), detected_format="jsonl",
        languages=["en"], topics=["Sports", "Politics and conflicts"], countries=["Germany"],
        max_per_topic=10, min_article_length=100, random_seed=42,
    )
...
def test_empty_languages_raises_value_error(tmp_path):
    ...
    with pytest.raises(ValueError):
        data_normalizer.normalise_articles(
            raw_path=str(p), detected_format="jsonl",
            languages=[], topics=["Sports"], countries=["Germany"],
            max_per_topic=10, min_article_length=100, random_seed=42,
        )
...
def test_empty_topics_raises_value_error(tmp_path):
    ...
    with pytest.raises(ValueError):
        data_normalizer.normalise_articles(
            raw_path=str(p), detected_format="jsonl",
            languages=["en"], topics=[], countries=["Germany"],
            max_per_topic=10, min_article_length=100, random_seed=42,
        )
...
def test_empty_countries_raises_value_error(tmp_path):
    ...
    with pytest.raises(ValueError):
        data_normalizer.normalise_articles(
            raw_path=str(p), detected_format="jsonl",
            languages=["en"], topics=["Sports"], countries=[],
            max_per_topic=10, min_article_length=100, random_seed=42,
        )
...
GUARANTEED_FIELDS = {"id", "event_id", "title", "text", "language",
                     "topic", "country", "date"}
...
def test_output_articles_have_all_guaranteed_schema_fields(tmp_path):
    ...
    assert GUARANTEED_FIELDS.issubset(articles[0].keys()), (
        f"Missing guaranteed schema fields: "
        f"{GUARANTEED_FIELDS - articles[0].keys()}"
    )
...
def test_max_per_topic_caps_group_size(tmp_path):
    ...
    records = [
        _wikinews_record(text=f"Unique body {i} " * 30, article_id=f"id{i}")
        for i in range(30)
    ]
    ...
    articles, _ = data_normalizer.normalise_articles(
        raw_path=str(p), detected_format="jsonl",
        languages=["en"], topics=["Sports"], countries=["Germany"],
        max_per_topic=5, min_article_length=10, random_seed=42,
    )
...
def test_sampling_is_deterministic_with_same_seed(tmp_path):
    ...
    records = [
        _wikinews_record(text=f"Unique body {i} " * 30, article_id=f"id{i:03d}")
        for i in range(30)
    ]
    p = _write_jsonl(tmp_path, records)
    kwargs = dict(
