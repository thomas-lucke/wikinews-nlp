...
def test_resolve_overlapping_entities_discards_offset_mismatch():
    # SPEC: _resolve_overlapping_entities — "Offset validation: ... if it does
    # not match, log a warning and discard the entity."
    text = "Hello Berlin today."
    entities = [
        # Offsets 0-5 point to "Hello", not "Berlin" — must be discarded.
        {"text": "Berlin", "label": "LOC", "start": 0, "end": 5, "score": 0.9},
    ]
    out = ner._resolve_overlapping_entities(entities, text)
    assert out == []
...
def test_run_ner_skips_articles_of_other_language(mock_ner_pipeline, sample_de_article):
    ...
    out = ner.run_ner(articles, mock_ner_pipeline, language="en",
...
def test_run_ner_renames_entity_group_to_label(mock_ner_pipeline, sample_en_article):
    ...
    mock_ner_pipeline.return_value = [
        {"word": "Berlin", "entity_group": "LOC",
         "score": 0.99, "start": 6, "end": 12},
    ]
    out = ner.run_ner([sample_en_article], mock_ner_pipeline, language="en",
                     chunk_size=400, chunk_overlap=50)
...
def test_run_ner_empty_pipeline_result_sets_empty_list(
    mock_ner_pipeline, sample_en_article,
):
...
def test_run_ner_per_article_error_does_not_crash(
    mock_ner_pipeline, sample_en_article, sample_de_article,
):
    ...
    try:
        ...
        pytest.fail(
            f"run_ner must catch per-article errors, got: {exc!r}"
        )
    assert out[0]["entities"] == []
...
EXPECTED_ENTITY_DF_COLS = {
    "article_id", "event_id", "title", "date",
    "language", "country", "topic",
...
def test_build_entity_dataframe_columns(sample_en_article):
    ...
    # language, country, topic, entity_text, entity_label, score".
...
def test_build_entity_dataframe_substitutes_id_for_empty_title():
    ...
    article = {
        "id": "X1", "event_id": None, "title": "", "date": None,
        "language": "en", "country": "germany", "topic": "sports",
        "entities": [
            {"text": "Berlin", "label": "LOC", "start": 0, "end": 6, "score": 0.9},
        ],
    }
...
def test_investigate_ner_errors_threshold(sample_en_article):
    ...
    df = ner.investigate_ner_errors(
        [sample_en_article], language="en",
        error_score_threshold=0.6, country=None,
    )
