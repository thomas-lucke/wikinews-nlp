...
def test_detect_format_jsonl_with_readme_present(tmp_path):
    ...
    (tmp_path / "wikinews.jsonl").write_text(json.dumps({"text": "hi"}) + "\n", encoding="utf-8")
...
def test_detect_format_exactly_80_percent_dominant(tmp_path):
    ...
    for i in range(4):
        (tmp_path / f"a{i}.jsonl").write_text(json.dumps({"text": "x"}) + "\n", encoding="utf-8")
...
def test_category_profile_sorts_by_count_descending(tmp_path):
    ...
    assert counts == sorted(counts, reverse=True), "category_profile must sort by count descending"
...
def test_category_profile_deduplicates_within_record(tmp_path):
    ...
    assert int(row["count"].iloc[0]) == 1, "Repeated category labels in one record must count once."
